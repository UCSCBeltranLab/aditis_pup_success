...
mod_binom_1_only_yr_born_in <- glmmTMB(is_one ~ ns(AgeYears, 3) + age_last_seen + total_resights + (1 | animalID_fct) + (1 | year_born),
                           data = intrinsic_variables,
                           family = binomial(link = "logit"))
summary(mod_binom_1_only_yr_born_in)
exp(fixef(mod_binom_1_only_yr_born_in)$cond) 
DHARMa::simulateResiduals(mod_binom_1_only_yr_born_in, plot = TRUE)
#residuals look great
check_collinearity(mod_binom_1_only_yr_born_in)
r2(mod_binom_1_only_yr_born_in)


mod_binom_1_only_all <- glmmTMB(is_one ~ ns(AgeYears, 3) + age_last_seen + total_resights + (1 | animalID_fct) + (1 | year_born) + (1 | season_fct),
                                       data = intrinsic_variables,
                                       family = binomial(link = "logit"))
summary(mod_binom_1_only_all)
exp(fixef(mod_binom_1_only_all)$cond) 
DHARMa::simulateResiduals(mod_binom_1_only_all, plot = TRUE)
#residuals look great
check_collinearity(mod_binom_1_only_all)
...
##Gamma model <1
mod_gamma_non1 <- glmmTMB(flipped_prop ~ ns(AgeYears, 3) + age_last_seen + total_resights + (1 | animalID_fct) + (1 | season_fct) + (1 | year_born),
...
##Piecewise model with only season pre- and post- senescent threshold
##lowest AIC
mod_piecewise_only_season <- glmmTMB(is_one ~ age10 : age_cat + age_last_seen + total_resights + (1 | season_fct),
                             family = binomial(link = "logit"),
                             data = intrinsic_variables)
summary(mod_piecewise_only_season)
exp(fixef(mod_piecewise_only_season)$cond) 
DHARMa::simulateResiduals(mod_piecewise_only_season, plot = TRUE)
#residuals look good here too
check_collinearity(mod_piecewise_only_season)

##Piecewise model with only animalID and season pre- and post- senescent threshold
mod_piecewise_both <- glmmTMB(is_one ~ age10 : age_cat + age_last_seen + total_resights + (1 | animalID) + (1 | season_fct),
                                     family = binomial(link = "logit"),
                                     data = intrinsic_variables)
summary(mod_piecewise_both)
exp(fixef(mod_piecewise_both)$cond) 
DHARMa::simulateResiduals(mod_piecewise_both, plot = TRUE)
#residuals look good here too
check_collinearity(mod_piecewise_both)

##Piecewise model with all random effects age pre- and post- senescent threshold
...
##Piecewise model with Gamma between 0 and 1
mod_piecewise_gamma <- glmmTMB(flipped_prop ~ age10 : age_cat + age_last_seen + total_resights + (1 | animalID_fct) + (1 | season_fct) + (1 | year_born_fct),
                               family = Gamma(link = "log"),
                               data = intrinsic_variables_sub)
summary(mod_piecewise_gamma)
exp(fixef(mod_piecewise_gamma)$cond) 
DHARMa::simulateResiduals(mod_piecewise_gamma, plot = TRUE)
#residuals look not the best but fine
check_collinearity(mod_piecewise_gamma)

##Create ANNUAL (i.e. rand effect of season only) predictions
pred_season <- ggpredict(
  mod_piecewise_only_season, 
...
##Piecewise model with only season pre- and post- senescent threshold for 2016-2025
...
mod_piecewise_gamma_2016_2025 <- glmmTMB(flipped_prop ~ age10 : age_cat + age_last_seen + total_resights + (1 | season_fct) + (1 | animalID_fct),
                                         family = Gamma(link = "log"),
                                         data = intrinsic_variables_sub_2016_2025)
summary(mod_piecewise_gamma_2016_2025)
exp(fixef(mod_piecewise_gamma_2016_2025)$cond) 
DHARMa::simulateResiduals(mod_piecewise_gamma_2016_2025, plot = TRUE)
#residuals look not the best but fine
check_collinearity(mod_piecewise_gamma_2016_2025)
